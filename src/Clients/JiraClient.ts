...
export class JiraClient implements ITfsClient {
	...
	public async update(settings: AgileTaskNotesSettings): Promise<void> {
		...
		let token = settings.jiraSettings.apiToken;
		if (existsSync(settings.jiraSettings.apiToken)) {
			try {
				token = readFileSync(settings.jiraSettings.apiToken, 'utf8').trim();
			} catch (error) {
		...
		if (settings.jiraSettings.authmode == 'basic') {
			const encoded64Key = Buffer.from(`${settings.jiraSettings.email}:${token}`).toString(
				'base64'
			);
